...
async function callLocalCorrector(text) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Local correction failed (${response.status}): ${message}`);
  }
...
function initSpeechRecognition() {
  ...
  recognition.onresult = (event) => {
    ...
    if (transcript) {
      ...
      inputTextarea.value = currentText ? `${currentText} ${transcript}` : transcript;
      updateCharCount();
      voiceStatus.textContent = 'Speech captured. You can submit or speak again.';
    }
  };

  recognition.onerror = (event) => {
    console.error('Speech recognition error', event);
    voiceStatus.textContent = 'Voice input error. Try again.';
    isListening = false;
  ...
  recognition.onend = () => {
    if (isListening) {
      isListening = false;
      voiceBtn.classList.remove('listening');
      voiceStatus.textContent = 'Voice input stopped. Tap mic to speak again.';
    }
  };
}
...
function showOutput(text, pronunciation, originalText) {
  correctedText = text;
  loadingContainer.classList.remove('active');
  errorContainer.classList.remove('active');
  ...
  pronunciationPanel.classList.remove('hidden');
...
function computeWordDiff(a, b) {
  // Simple Myers-like diff for words
  const n = a.length;
  const m = b.length;
...
async function handleSubmit() {
  ...
  try {
    const result = await callLocalCorrector(text);
    showOutput(result.corrected || '', result.pronunciation || '', text);
  } catch (err) {
    console.error('Correction Error:', err);
    showError('स्थानिक सुधारणा अयशस्वी झाली.', 'स्थानीय सुधार विफल हुआ।', `Local error: ${err.message}`);
  } finally {
